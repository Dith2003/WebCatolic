<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Alumno</title>

    <!-- font awesome cdn link  -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css">

    <!-- custom css file link  -->
    <link rel="stylesheet" href="css/style.css">

</head>
<body>
    
<div class="container">

<header>

    <a href="#" class="logo">Web<span>Catolic</span></a>

    <div id="menu" class="fas fa-bars"></div>

    <nav class="navbar">
        <a href="Index.php">Inicio</a>
        <a href="Noticias.php">Noticias</a>
        <a href="Inscripciones.php">Inscripciones</a>
        <a href="Anuncios.php">Anuncios</a>
        <a href="Alumno.php">Alumno</a>
        <a href="/WebCatolic/Index.php">Cerrar Sesión</a>
    </nav>

</header>

<h1 class="heading"> Secciones </h1>

<!-- course section  -->

<section class="course">

    <div class="box">
        <img src="images/course-2.svg" alt="">
        <h3>Confirmacion</h3>
        <a href="Confirmacion/Confirmacion.php" class="btn">Visitar</a>
    </div>

    <div class="box">
        <img src="images/course-3.svg" alt="">
        <h3>Primera comunion</h3>
        <a href="Primera-C/Primera-C.php" class="btn">Visitar</a>
    </div>

    <div class="box">
        <img src="images/course-5.svg" alt="">
        <h3>Avisos</h3>
        <a href="Avisos.php" class="btn">Visitar</a>
    </div>


</section>


<!-- footer section  -->

<section class="footer">

    <div class="credit"> created by <span> mr. web designer </span> | all rights reserved </div>

</section>

</div>



<!-- custom js file link -->
<script src="js/script.js"></script>

</body>
</html>